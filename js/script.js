	...
	$.each(data, function(i, tabledata) {
		...
		if (data[i].date.indexOf('closed') == -1) {
			renderHtml +=
				...
				'</td><td> <a href="' +
				data[i].link +
				'" target="_blank"> More Info </a></td><td class="table-success">' +
				parsedDate +
				'</td><td></tr>';
		} else {
			renderHtml +=
				...
				'</td><td> <a href="' +
				data[i].link +
				'" target="_blank"> More Info </a></td><td class="table-danger">' +
				parsedDate +
				'</td><td></tr>';
		}
...
$(document).ready(function() {
	// GET JSON DATA
	const urls = ['json/ausdata.json', 'json/creativevic.json'];
	urls.forEach((url) => {
		$.ajax({
			type: 'GET',
			url: url,
